...
generates MG and Pythia8 cards from templates and submits HTCondor jobs per (mass, coupling) point defined optionally.
Instead of producing a separate sample for every cah value, one sample is produced per (mass, Z channel) at cah_ref
and MG reweights the events to all other cah values.
...
def write_reweight_card(cah_list: list, gen_dir: str, ma: float) -> str:
    """
    - Writes reweight card for MadGraph according to a given parameter
    - Returns:
    path of the reweight card where rwgt_1 -> cah_list[0], etc
    - Args:
    cah_list: list, all cah values
    gen_dir: str, generation directory
    ma: float, alp mass
    """
    content    = ''
    
    for i, cah_value in enumerate(cah_list):
        content    +=    f'launch --rwgt_name=cah{i}\n'
        content    +=    f'set CAH {cah_value}\n'
    
    m	= f"{ma}".replace(".", "p")
    path    = gen_dir + f'reweight_card_m{m}.dat'
...
def write_mg_card(channel: str, ma: float, cah_list: list, gen_dir: str, mg_out_dir: str) -> str:
    ...
    reweight_card	= write_reweight_card(cah_list, gen_dir, ma)
    ...
    content +=	"reweight=ON\n"
    content +=	"done\n\n"
    content +=	f"{reweight_card}\n\n"
    content	+=	"set store_rwgt_info True\n"
...
def write_mg_validation_card(channel: str, ma: float, cah:float, gen_dir: str, mg_out_dir: str) -> str:
    ...
    m		= f"{ma}".replace(".", "p")
    path    = gen_dir + f"mg_param_m{m}_val.txt"
...
def write_job(condor_job_dir: str, mg_card: str, mg_out_dir: str, pythia_card: str, delphes_out: str, skip_mg=False) -> str:
    """
...
def run(dry_run: bool=True, skip_existing: bool=True, use_condor: bool=True, skip_mg=False):
    """
    ...
    for channel, (ma, cah_list) in itertools.product(z_channels.keys(), alp_scan.items()):
        ...
        if dry_run:
            ...
            if ret == 0:
                print(f"  Local run complete!")
            else:
                print(f"  [ERROR] local run returned {ret}")
            nsubmit += 1
 
    print(f"\nSubmitted: {nsubmit} | Skipped: {nskipped}")
    
def run_validation(channel: str, ma: float, cah: float, dry_run: bool = True, use_condor: bool = True, skip_mg=False):
    """
    ...
    script      = write_job_validation(condor_job_dir, mg_card, mg_out_dir, pythia_card, delphes_out, skip_mg=skip_mg)
    cfg_path    = write_condor_config(condor_job_dir, script)
    ...
    if ma in alp_scan and cah in alp_scan[ma]:
        ...
        idx			= alp_scan[ma].index(cah) + 1   # +1 because ind(0) = cah_ref
    ...
    if dry_run:
        ...
        if ret==0:
            ...
            print(f" [ERROR] local execution returned {ret}")
        
if __name__ == "__main__":
    run(dry_run=False, skip_existing=False, use_condor=use_condor, skip_mg=skip_mg)
    
    for channel, (ma, cah_list) in itertools.product(z_channels.keys(), alp_scan.items()):
        if cah_val not in cah_list:
            continue
        run_validation(channel, ma, cah_val, dry_run=False, use_condor=use_condor, skip_mg=skip_mg)