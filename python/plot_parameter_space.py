...
def alp_width_from_ctau(ctau_mm: float) -> float:
    ctau_natural = ctau_mm / HBAR_C_GEV_MM
...
def build_grid(channels_use):
    ...
    for i_c, ctau_label in enumerate(CTAU_LABELS):
        for i_m, malp_val in enumerate(malp):
            for channel in channels_use:
                banner = events_dir(channel, malp_val, ctau_label) / "run_01_tag_1_banner.txt"
    ...
    width_arr = np.array([alp_width_from_ctau(c) for c in ctau_mm_arr])
...
def _draw_heatmap(vals, x_ticklabels, y_ticklabels, x_axis_label, y_axis_label,
    ...
    if np.all(np.isnan(plot_vals)):
        ...
        return
    im = ax.imshow(
        plot_vals, aspect="auto", origin="lower",
    ...
    title = (
        ...
        + r"; $N=\mathcal{L}\sigma f_{\mathrm{dec}}$ (geometric, not selected)"
    )
...
def make_plot_2d_ctau(n_events, malp, out_name, subtitle):
    x_ticklabels = [f"{m:g}" for m in malp]
    _draw_heatmap(n_events, x_ticklabels, CTAU_LABELS,
                  r"$m_a$ [GeV]", r"$c\tau_a$",
                  out_plot_path, out_name, subtitle=subtitle)

def make_plot_2d_gamma(n_events, width_arr, malp, out_name, subtitle):
    y_ticklabels = [f"{m:g}" for m in malp]
    x_ticklabels = [f"{w:.2e}" for w in width_arr]
    _draw_heatmap(n_events.T, x_ticklabels, y_ticklabels,
                  r"$\Gamma_a$ [GeV]", r"$m_a$ [GeV]",
                  out_plot_path, out_name, subtitle=subtitle)

if __name__ == "__main__":
    jobs = [
        (["ee"], "alp_parameter_space_ctau_ee", r"$Z\to e^+e^-$"),
        (["mumu"], "alp_parameter_space_ctau_mumu", r"$Z\to\mu^+\mu^-$"),
        (["ee", "mumu"], "alp_parameter_space_ctau", r"$Z\to\ell^+\ell^-$"),
    ]
    for chans, name, sub in jobs:
        n_events, xsec_pb, fdec, ctau_mm_arr, width_arr = build_grid(chans)
        print(f"\n=== {name}  channels={chans} ===")
        print("m_a [GeV]  |  ctau  |  fdec  |  xsec [pb]  |  n_events")
        for i_c, c_label in enumerate(CTAU_LABELS):
            for i_m, ma in enumerate(malp):
                fd, xs, nev = fdec[i_c, i_m], xsec_pb[i_c, i_m], n_events[i_c, i_m]
        ...
        make_plot_2d_ctau(n_events, malp, name, sub)
        if chans == ["ee", "mumu"]:
            make_plot_2d_gamma(n_events, width_arr, malp,
                               "alp_parameter_space_gamma", r"$Z\to\ell^+\ell^-$")